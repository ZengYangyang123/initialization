<template>
  <div class="w-screen h-screen bg-[#F7F7F7] p-[20px]">
    <isHead class="flex w-[100vw] h-[68px] bg-[white] fixed left-0 top-0" style="box-shadow: 0 0 10px #ccc;"></isHead>
    <div
      ...
      <div class="w-[223px]">
        <a-dropdown :trigger="['click']">
          <a class="flex items-center" @click.prevent>
            <span class="text-sm">活的数据</span>
            <Icon icon="ooui:expand" class="text-[16px] ml-[5px] text-black"></Icon>
          </a>
          <template #overlay>
            <a-menu class="w-[274px]" style="margin-left: -30px !important; padding: 12px 0 !important">
              <div class="w-full px-[6px] relative">
                <a-input v-model:value="formState.searchFor" class="h-[36px] pl-[30px] rounded-[4px]" placeholder="搜索" />
                <Icon icon="simple-line-icons:magnifier" class="absolute top-[12px] left-[14px]" />
              </div>
              <div class="h-[500px] overflow-y-auto scroller">
                <a-menu-item v-for="item in dataxiang?.data.rows" :key="item.id" class="h-[60px] overflow-hidden">
                  <div class="flex items-center h-full">
                    <div class="w-[32px] h-[32px] rounded-[50%] mr-[10px] overflow-hidden">
                      <img :src="`http://api.cc0820.top:1024${item.cover}`" alt="" class="w-full h-full" />
                    </div>
                    <p>{{ item.name }}</p>
                  </div>
                </a-menu-item>
      ...
      <div class="w-[264px] text-[#C0C4CC] text-sm h-full flex items-center justify-end relative">
        <!-- 看板视图 -->
        <button class="flex items-center element h-full mr-[5px]" v-if="activeKey === '1'">
          <Icon icon="prime:book"></Icon>
          <span class="ml-[5px]">看板视图</span>
        </button>
        <!-- 筛选 -->
        <a-dropdown :trigger="['click']" class="text-[#409EFF] flex items-center Smallhand mr-[5px] h-full"
          v-if="activeKey === '1'">
        ...
        <!-- 成员 -->
        <div class="w-[36px] flex items-center text-[#409EFF] Smallhand mr-[5px] h-full" @click="showModal">
        ...
        <div>
          <a-modal v-model:open="open" title="项目成员" @ok="handleOk">
            <a-form-item name="username" class="relative mb-0">
              <a-input v-model:value="formState.username" placeholder="请输入用户名或邮箱查找"
                class="el-input__inner pl-[26px] w-full h-[40px] text-[#606266]" />
              <Icon icon="simple-line-icons:magnifier" class="absolute top-[50%] left-[8px] translate-y-[-50%]" />
            </a-form-item>
            <div class="pt-[10px] min-h-[300px]">
              <div class="py-[12px] flex justify-between items-center" style="border-top: 1px solid #e8e8e8">
                <div class="w-[32px] h-[32px] rounded-[50%] overflow-hidden">
                  <img src="" alt="" class="w-full h-full" />
                </div>
                <div class="w-[380px] text-[14px]">
                  <p style="line-height: 22px">活的数据</p>
                  <p style="line-height: 22px">活的数据</p>
                </div>
                <div>
                  <Icon icon=""></Icon>
        ...
        <!-- 菜单 -->
        <a-dropdown :trigger="['click']" class=" flex items-center text-[#409EFF] Smallhand h-full relative z-[3]">
          <a class="ant-dropdown-link" @click.prevent>
            <Icon icon="line-md:menu-fold-left" :rotate="2" />
            <span class="ml-[5px]">菜单</span>
          </a>
          <template #overlay>
            <a-menu class="w-[256px] relative z-[3]" style="right: -30px;top: 10px; padding: 12px;">
              <!-- 菜单弹窗 -->
    ...
    <div class="w-full p-[20px] bg-[#F7F7F7]">
      ...
      <Version v-if="activeKey === '4'" style="height: calc(100vh - 192px)"></Version>
      <Schedule v-if="activeKey === '5'" style="height: calc(100vh - 192px)"></Schedule>
    </div>
  </div>
  <!-- 项目设置 -->
  <isProject v-if="setUp" @chilFun="csFun" style="transition: all 0.8s"></isProject>
</template>
<script setup lang="ts">
import { Icon } from "@iconify/vue";
import { reactive, ref } from "vue";
import { ongoingProject } from "@/service/index";
...
:where(.css-dev-only-do-not-override-eq3tly).ant-tabs .ant-tabs-tab.ant-tabs-tab-active .ant-tabs-tab-btn {
  color: #000 !important;
  text-shadow: none;
}
...
:where(.css-dev-only-do-not-override-eq3tly).ant-form-horizontal .ant-form-item-label {
  margin-right: 16px;
}
...
:where(.css-dev-only-do-not-override-eq3tly).ant-form-item .ant-form-item-label>label::after {
  content: none;
}
...
/* 重置悬停 */
:where(.css-dev-only-do-not-override-eq3tly).ant-btn-default:not( :disabled):hover {
  color: #409eff;
  border-color: #c6e2ff;
