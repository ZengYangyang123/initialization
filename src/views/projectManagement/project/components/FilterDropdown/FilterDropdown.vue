  ...
  <div class="w-[305px] z-[3] mt-[12px]  pr-[12px] text-[14px]">
    <a-form ref="formRef" :model="formState" name="basic" autocomplete="off" @finish="onFinish"
      ...
      <a-form-item label="标题" name="title" class="flex justify-end">
        <a-input v-model:value="formState.title" placeholder="搜索任务标题" class="w-[200px] h-[40px]" />
      </a-form-item>
      <a-form-item label="执行者" name="performer" class="flex justify-end">
        <a-select v-model:value="formState.performer" placeholder="请选择" class="h-[40px]" style="width: 200px">
      ...
      <a-form-item label="创建者" name="creator" class="flex justify-end">
        <a-select v-model:value="formState.creator" placeholder="请选择" class="h-[40px]" style="width: 200px">
          <a-select-option value="1">活的数据</a-select-option>
          <a-select-option value="2">活的数据</a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item label="是否完成" name="CompletedOrNot" class="flex justify-end">
        <a-select v-model:value="formState.CompletedOrNot" class="h-[40px]" style="width: 200px">
      ...
      <a-form-item label="优先级" name="priority" class="flex justify-end">
        <a-select v-model:value="formState.priority" placeholder="请选择" class="h-[40px]" style="width: 200px">
          <a-select-option value="1">普通</a-select-option>
          <a-select-option value="2">紧急</a-select-option>
          <a-select-option value="3">非常紧急</a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item label="执行状态" name="ExecutionStatus" class="flex justify-end">
        <a-select v-model:value="formState.ExecutionStatus" placeholder="请选择" class="h-[40px]" style="width: 200px">
      ...
      <a-form-item class="text-sm yangshi">
        <a-button type="primary" class="text-[#fff] bg-[#409EFF] border-[409EFF] w-[70px] h-[40px]"
          @click="onSubmit">搜索</a-button>
        <a-button style="margin-left: 10px" class="w-[70px] h-[40px]" @click="resetForm">重置</a-button>
      </a-form-item>
    </a-form>
    <!-- 小三角 -->
    <div class="triangle absolute top-[-7px] left-[50%] translate-x-[-50%]"></div>
  </div>
</template>
